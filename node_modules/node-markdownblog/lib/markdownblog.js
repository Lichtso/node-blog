...
exports.createNewArticle = function(name) {
  ...
  app.maxID++;
  var now = new Date();    
  var newName = curID + '-' + slug;
  var newFile = this.path.articles + '/' + newName + '/';
...
exports.parseFile = function(b) {
  var directory = this.path.articles+'/'+b+'/';
  var content = fs.readFileSync(directory+'article.html')+'';
  if(content.substr(0, 5) != '<!--\n' || content.indexOf('-->') == -1) return null;
  var c=content.split('-->'),header=c[0].substr(5),e=c[1],f=y.decode(header),g=[],h=f.Sources||{};
  var name=(/Title:(.*)/.exec(header)[1]+'').replace('Title:','').replace(/^\s\s*/, '').replace(/\s\s*$/, '');
  var id=b.split('-')[0]*1,l=f.Tags.split(', ').sort(), m=new Date(f.Date),o=[];
...
exports.processArticles = function() {
  for(var a = this.files.articles.length; a --; ) {
    ...
    if(!article) continue;
    if(this.maxID < article.id) this.maxID = article.id;
